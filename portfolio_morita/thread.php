...
if(!empty($_POST)){
    if($_POST['name'] !== '' && $_POST['p_question'] !== ''){
        $questions = $db->prepare('INSERT INTO thread SET title = ?,member_name = ?, message = ?, created = NOW()');
...
$count = $db->query('SELECT COUNT(*) as cnt FROM thread');
$cnt = $count->fetch();
...
$start = ($page -1)*10;
/*DBから投稿を取得する*/
$threads = $db ->prepare('SELECT * FROM thread ORDER BY modified LIMIT ?,10');
    ...
    <div class="head">
        ...
        <div id="head-right">
            <ul>
                ...
                <li><a href="logout_p.php">ユーザー情報</a></li>
                <li><a href="logout_p.php">ログアウト</a></li>
            </ul>
        </div>
    <!--ヘッダー終了-->
    <!--コンテンツ開始-->
    <div id="content">
        ...
        foreach($threads as $thread){
           ...
           /* URLパラメータを使って、該当IDのeach_thread.phpに遷移する*/
            ...
            echo '<td><a href="each_thread.php?thread_id='.htmlspecialchars($thread['id']).'">詳細</a>'.'  '.'</td>';
        ...
        <form action="" method="post">
            ...
            <p><input type="submit" value="スレッドを立てる"></p>
        </form>
    </div>
    <!--コンテンツ終了-->
    <!--フッター終了-->
