...
*/
/*スレッドを立てる処理
Rubyに関するスレッドを立てるページなので、thread_id='Ruby'に指定する。*/
if(!empty($_POST)){
    if($_POST['name'] !== '' && $_POST['p_question'] !== ''){
        $questions = $db->prepare('INSERT INTO thread SET title = ?,member_name = ?, message = ?, created = NOW(),thread_id ="ruby",p_language="Ruby"');
        $questions->execute(array($_POST['title'],$member['name'],$_POST['p_question']));
        header('Location:thread_ruby.php');
        exit();
    }
}
