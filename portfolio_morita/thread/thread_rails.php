...
$start = ($page -1)*10;
/*DBから投稿を取得する*/
$threads = $db ->prepare('SELECT * FROM thread WHERE thread_id = "rails" ORDER BY modified LIMIT ?,10');
...
<head>
	...
	<title>Ruby and Rails質問スレッド一覧画面</title>
    ...
    <div id="content">
       ...
       <?php 
        ...
        <div>
            <ul class="paging">
                <?php if($page >1):?>
                    <li><a href="thread_rails.php?page_thread=<?php print(htmlspecialchars($page -1));?>">前のページへ</a></li>
                <?php else:?>
                    <li>前のページへ</li>
                <?php endif;?>
                <?php if($page < $maxPage):?>
                    <li><a href="thread_rails.php?page_thread=<?php print(htmlspecialchars($page+1));?>">次のページへ</a></li>
                <?php else:?>
                    <li>次のページへ</li>    
                <?php endif;?>
            </ul>
