...
*/
/*スレッドを立てる処理
railsに関するスレッドを立てるページなので、thread_id='rails'に指定する。*/
if(!empty($_POST)){
    if($_POST['name'] !== '' && $_POST['p_question'] !== ''){
        $questions = $db->prepare('INSERT INTO thread SET title = ?,member_name = ?, message = ?, created = NOW(),thread_id ="rails",p_language="Ruby on Rails"');
        $questions->execute(array($_POST['title'],$member['name'],$_POST['p_question']));
        header('Location:thread_rails.php');
        exit();
    }
}
...
<head>
	...
	<title>Ruby on Rails質問スレッド一覧画面</title>
