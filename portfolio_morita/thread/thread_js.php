...
*/
/*スレッドを立てる処理
JSに関するスレッドを立てるページなので、thread_id='js'に指定する。*/
if(!empty($_POST)){
    if($_POST['name'] !== '' && $_POST['p_question'] !== ''){
        $questions = $db->prepare('INSERT INTO thread SET title = ?,member_name = ?, message = ?, created = NOW(),thread_id ="js",p_language="JavaScript"');
        $questions->execute(array($_POST['title'],$member['name'],$_POST['p_question']));
        header('Location:thread_js.php');
        exit();
    }
}
...
<head>
	...
	<title>JavaScript 質問スレッド一覧画面</title>
