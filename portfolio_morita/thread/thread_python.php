...
$search_sql='SELECT * FROM thread WHERE thread_id = "python" AND title like :search_word';
$search_stmt=$db->prepare($search_sql);
$search_stmt->bindParam(':search_word',$search_word,PDO::PARAM_STR);
$search_stmt->execute();

/*DBから、今何件スレッドが立っているか、数をとってくる*/
$count = $db->query('SELECT COUNT(*) as cnt FROM thread WHERE thread_id = "python"');
$cnt = $count->fetch();
...
$start = ($page -1)*10;
/*DBから投稿を取得する*/
$threads = $db ->prepare('SELECT * FROM thread WHERE thread_id = "python" ORDER BY modified DESC LIMIT ?,10');
...
<!--ヘッダー終了-->
    ...
    <div id="content">
       ...
       <?php 
        ...
        echo '</table>';
        print '<br><br>';
        print '<a class="button_link" href="thread_python.php">スレッド一覧に戻る</a>';
        ...
        ?>
            ...
            <?php if(empty($_POST) || $_POST['searchThreadTitle'] ===''):?>
                ...
                <div>
                    <ul class="paging">
                        <?php if($page >1):?>
                            <li><a class="button_link" href="thread_python.php?page_thread=<?php print(htmlspecialchars($page -1));?>">前のページへ</a></li>
                        <?php else:?>
                            <li>前のページへ</li>
                        <?php endif;?>
                        <?php if($page < $maxPage):?>
                            <li><a class="button_link" href="thread_python.php?page_thread=<?php print(htmlspecialchars($page+1));?>">次のページへ</a></li>
                        <?php else:?>
                            <li>次のページへ</li>    
                        <?php endif;?>
                    </ul>
