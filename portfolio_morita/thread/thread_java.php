...
*/
/*スレッドを立てる処理
Javaに関するスレッドを立てるページなので、thread_id='java'に指定する。*/
if(!empty($_POST)){
    if($_POST['name'] !== '' && $_POST['p_question'] !== ''){
        $questions = $db->prepare('INSERT INTO thread SET title = ?,member_name = ?, message = ?, created = NOW(),thread_id ="java",p_language="Java"');
        $questions->execute(array($_POST['title'],$member['name'],$_POST['p_question']));
        header('Location:thread_java.php');
        exit();
    }
}
    ...
    <div id="content">
        ...
        foreach($threads as $thread){
           ...
           /* URLパラメータを使って、該当IDのeach_thread.phpに遷移する*/
            ...
            /*ログインしている本人が書き込んだスレッドのみ削除できるようにする*/
