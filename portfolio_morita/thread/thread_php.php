...
*/
/*スレッドを立てる処理
PHPに関するスレッドを立てるページなので、thread_id='PHP'に指定する。*/
if(!empty($_POST)){
    if($_POST['name'] !== '' && $_POST['p_question'] !== ''){
        $questions = $db->prepare('INSERT INTO thread SET title = ?,member_name = ?, message = ?, created = NOW(),thread_id ="php",p_language="PHP"');
        $questions->execute(array($_POST['title'],$member['name'],$_POST['p_question']));
        header('Location:thread_php.php');
        exit();
    }
}
